 ...
 */
public class DateMonthPickDialogUtil implements OnDateChangedListener, OnTimeChangedListener {
	 ...
	 */
	public DateMonthPickDialogUtil(Context activity) {
		this.activity = activity;
		Calendar calendar = Calendar.getInstance();
		this.initDateTime = calendar.get(Calendar.YEAR) + "年" + (calendar.get(Calendar.MONTH) + 1) + "月" + calendar.get(Calendar.DAY_OF_MONTH) + "日 ";

	}

	public void init(DatePicker datePicker, EditText inputDate) {
		...
		if (TextUtils.isEmpty(date)) {// 未设置时间
			...
			initDateTime = Tools.getDateTime(date);
		}
		if (!(null == initDateTime || "".equals(initDateTime))) {
			calendar = this.getCalendarByInintData(initDateTime);
		} else {
			initDateTime = calendar.get(Calendar.YEAR) + "年" + (calendar.get(Calendar.MONTH) + 1) + "月" + calendar.get(Calendar.DAY_OF_MONTH) + "日 ";
		}
