...
/**
 * @package: com.xiangxun.xacity.ui.materiel.detail
 * @ClassName: RequirementManageDetailActivity.java
 * @Description: 需求管理详情待办任务页面
 * @author: HanGJ
 * @date: 2016-2-25 下午1:33:59
 */
public class RequirementManageTodoDetailActivity extends BaseActivity implements OnClickListener, SelectItemClick {
	...
	public void initData() {
		...
		if (childAt == 0) {
			...
			tabtopView.setTabText("计划详情", "计划审核记录");
			tabtopView.setTabMiddleText("计划需求列表");
		}
		...
		MaterielDemandBean demandBean = (MaterielDemandBean) getIntent().getSerializableExtra("bean");
	...
	private void RequestList() {
		if (childAt == 0) {// 项目管理
			...
			if (isMiddleFlag) {// 需求计划信息网络请求
				DcNetWorkUtils.getMaterielDemandFormListList(this, handler, account, password, code);
			} else {// 需求计划审核记录网络请求
				DcNetWorkUtils.getMaterielDemandRecordListList(this, handler, account, password, id);
			}
		}
	}

	private void setViewData(MaterielDemandBean demandBean) {
		if (demandBean != null) {
			if (childAt == 0) {// 项目管理
				...
				tvTitle03.setText(Html.fromHtml("操作账号: "));
				tvTitle04.setText(Html.fromHtml("需求时限: "));
				tvTitle05.setText(Html.fromHtml("计划月份: "));
				tvTitle06.setText("计划编号: ");
				tvTitle07.setText("添加时间: ");
				...
				tvContent03.setText(Tools.isEmpty(demandBean.operator));
				tvContent04.setText(demandBean.deadtime != null && demandBean.deadtime.length() > 0 ? Tools.isEmpty(MyUtils.StringToDate(demandBean.deadtime)) : "");
				tvContent05.setText(demandBean.month != null && demandBean.month.length() > 0 ? Tools.isEmpty(MyUtils.StringToDate(demandBean.month)) : "");
				tvContent06.setText(Tools.isEmpty(demandBean.code));
				tvContent07.setText(Tools.isEmpty(demandBean.addtime));
