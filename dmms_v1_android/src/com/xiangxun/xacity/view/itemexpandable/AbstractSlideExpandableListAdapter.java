...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
	...
	private View lastBtn = null;// add by liweiping
	...
	public AbstractSlideExpandableListAdapter(ListAdapter wrapped) {
		...
		mRotateUpAnim2 = new RotateAnimation(0.0f, -180.0f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
		mRotateUpAnim2.setFillAfter(true);
		mRotateDownAnim2 = new RotateAnimation(-180.0f, 0.0f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
	 ...
	 */
	// public abstract View getExpandParentButton(View parent);
	...
	private void enableFor(final View button, final View target, final int position) {
		...
		if (height == -1) {
			...
			updateExpandable(target, position, button);
		} else {
			updateExpandable(target, position, button);
		}
		...
		// 点击item展开列表
		// ((View) button.getParent()).setOnClickListener(new OnClickListener()
		// {
		//
		// @Override
	...
	private void updateExpandable(View target, int position, View button) {
		...
		if (openItems.get(position)) {
			...
			button.setBackgroundResource(R.drawable.ex_down);
			button.startAnimation(mRotateUpAnim2);
		} else {
			target.setVisibility(View.GONE);
			params.bottomMargin = 0 - viewHeights.get(position);
			button.setBackgroundResource(R.drawable.ex_down_def);
			button.startAnimation(mRotateDownAnim2);
		}
	}
